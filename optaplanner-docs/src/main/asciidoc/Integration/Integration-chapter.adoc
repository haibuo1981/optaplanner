...
** Some examples expose a `Parametrization` object, which defines the weight for each score constraint.
The human planner can then tweak those weights at runtime.
